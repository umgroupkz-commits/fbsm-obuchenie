...
window.pgPayroll = function(){
  ...
  <div class="card"><div class="ct">🧮 Бухгалтеры — 425 540 ₸ оклад при норме 22 раб. дней</div>
    <div class="tw-wrap"><table>
      <tr><th>Сотрудник</th><th>Смен</th><th>Оклад</th><th>Премия</th><th>Удержания</th><th>К выплате</th><th></th></tr>
      <tr><td><b>Пак Марина</b></td><td>22</td><td>${fmtN(425540)}</td><td>—</td><td>—</td>
        <td style="font-weight:700;color:var(--blue)">${fmtN(425540)}</td>
        <td style="white-space:nowrap">
          <button class="btn bsm" id="uch-korr-buh" onclick="uchKorr('Пак Марина')" title="Корректировка">💵</button>
          <button class="btn bsm" id="uch-kvit-buh" onclick="uchKvitokBuh()" title="Квиток">🖨️</button></td></tr>
    </table></div>
    <div class="rnote">В графике не стоит: оклад платится по норме рабочих дней месяца. Пропуски — числом смен
      в корректировке, оклад станет пропорционально меньше. Оклад указан с налогами: пенсионные, ИПН и ВОСМС
      вносятся в корректировке.</div></div>
  <div class="card"><div class="ct">🧑‍💼 HR-менеджеры — 300 000 ₸ оклад при норме 22 раб. дней</div>
    <div class="alert ${_vakPodtv===null?'ae':'ai'}" style="display:block;font-size:12.5px">Закрытых вакансий:
      в Битриксе <b>${VAK_SPISOK.length}</b>, подтверждено
      <b>${_vakPodtv===null?'ещё нет — премия не начислена':_vakPodtv}</b>.
      <a href="#" onclick="pgVakansii();return false">Открыть вакансии</a></div>
    <div class="tw-wrap"><table>
      <tr><th>Сотрудник</th><th>Смен</th><th>Оклад</th><th>За вакансии</th><th>Премия</th><th>Удержания</th><th>К выплате</th><th></th></tr>
      <tr><td><b>Дундукова Елена</b></td><td>22</td><td>${fmtN(300000)}</td>
        <td>${vakPremiya()>0?'+'+fmtN(vakPremiya()):'—'}</td><td>—</td><td>—</td>
        <td style="font-weight:700;color:var(--blue)">${fmtN(300000+vakPremiya())}</td>
        <td style="white-space:nowrap">
          <button class="btn bsm" id="uch-korr-hr" onclick="uchKorr('Дундукова Елена')" title="Корректировка">💵</button>
          <button class="btn bsm" id="uch-kvit-hr" onclick="uchKvitokHr()" title="Квиток">🖨️</button></td></tr>
    </table></div>
    <div class="rnote">Сверх оклада — 40 000 ₸ за каждую закрытую вакансию сверх пяти, по числу, которое подтвердил
      главный администратор. Работает неофициально: налоги не удерживаются.</div></div>
...
// ── вакансии HR ──
// Как в боевой · 5: закрытая вакансия — карточка воронки найма, впервые перешедшая
// в этом месяце на «В штате». Число берётся из Битрикса, но в зарплату идёт только
// то, что подтвердил главный администратор: до подтверждения премия равна нулю.
const VAK_OKLAD = 300000, VAK_POROG = 5, VAK_ZA = 40000;
const VAK_SPISOK = [
  ['Продавец-кассир, Республика', '3 сентября 2026'],
  ['Продавец-кассир, Манаса', '8 сентября 2026'],
  ['Раннер, Алматы', '11 сентября 2026'],
  ['Стример, студия', '15 сентября 2026'],
  ['Продавец-кассир, Асфендиярова', '18 сентября 2026'],
  ['Кладовщик, склад Пушкина', '19 сентября 2026'],
  ['Продавец-кассир, Караганда', '22 сентября 2026'],
  ['Менеджер интернет-магазина', '23 сентября 2026'],
  ['Продавец-кассир, Астана', '24 сентября 2026'],
];
let _vakPodtv = null;
const vakPremiya = () => _vakPodtv === null ? 0 : Math.max(0, _vakPodtv - VAK_POROG) * VAK_ZA;
window.pgVakansii = function(){
  nav('pgVakansii'); setTitle(CU.role==='hr' ? 'Вакансии' : 'Вакансии HR'); setBadge('');
  const n = VAK_SPISOK.length, est = _vakPodtv !== null;
  setContent(`
  <div class="card">
    ...
    <div class="g3" style="margin:1rem 0">
      <div class="metric"><div class="ml">В Битриксе</div><div class="mv">${n}</div></div>
      <div class="metric"><div class="ml">Подтверждено</div><div class="mv">${est ? _vakPodtv : '—'}</div></div>
      <div class="metric"><div class="ml">Премия HR</div><div class="mv">${est ? fmtN(vakPremiya()) : '—'}</div></div>
    </div>
    <div style="font-size:12.5px;color:var(--muted);margin-bottom:.75rem">Закрытая вакансия — карточка воронки найма
      ...
      за каждую сверх — ${fmtN(VAK_ZA)} ₸. В зарплату идёт подтверждённое число.</div>
    <div class="tw-wrap"><table><tr><th>Карточка</th><th>В штате с</th></tr>
      ${VAK_SPISOK.map(v=>`<tr><td><a href="#" onclick="return false" style="color:var(--blue)">${esc(v[0])}</a></td>
        <td>${v[1]}</td></tr>`).join('')}
    </table></div>
    ${est?`<div class="alert as" style="display:block;margin-top:1rem;font-size:13px">✓ Подтверждено: <b>${_vakPodtv}</b>
      — Станков Андрей, 24 сентября 2026</div>`:''}
    ${CU.role==='admin'?`<div class="card" style="margin-top:1rem;max-width:460px"><div class="ct">Подтвердить число</div>
      <div class="fr"><div class="fg"><label class="fl">Закрыто вакансий</label>
        <input type="number" class="fc" id="uch-vak-n" value="${est?_vakPodtv:n}"></div></div>
      <div class="fg"><label class="fl">Комментарий (если число отличается от Битрикса)</label>
        <input type="text" class="fc" placeholder="например: одна карточка закрыта повторно"></div>
      <button class="btn bp tr-tyk" id="uch-vak-podtv" onclick="uchVakPodtv()">Подтвердить</button>
      ${est?' <button class="btn" id="uch-vak-snyat" onclick="uchVakSnyat()">Снять подтверждение</button>':''}
      <div class="rnote">Закрытый месяц не правится: если зарплата за месяц уже закрыта, число не меняется.</div></div>`:''}
  </div>`);
};
window.uchVakPodtv = function(){
  const v = parseInt(document.getElementById('uch-vak-n').value, 10);
  _vakPodtv = isFinite(v) && v >= 0 ? v : VAK_SPISOK.length;
  tr.sobytie('act:vak-podtv'); pgVakansii();
};
window.uchVakSnyat = function(){ _vakPodtv = null; tr.sobytie('act:vak-snyal'); pgVakansii(); };
...
// Бухгалтер и HR-менеджер в графике не стоят: оклад платится по норме месяца.
// У HR сверху к нему — премия за вакансии, и только по подтверждённому числу.
function uchKvitokOklad(dolzhnost, oklad, stroki, pripiska){
  ...
  ov.innerHTML=`<div class="ovc">
    ...
    <div style="border:1px solid var(--bd);border-radius:var(--rs);padding:1.25rem">
      ...
      <table><tr><th>Составляющая ЗП</th><th>Сумма</th></tr>
        <tr><td>Оклад (22 из 22 раб. дней × ${fmtN(oklad)})</td><td>${fmtN(oklad)}</td></tr>
        <tr><td>Норма месяца</td><td>22 раб. дня</td></tr>
        ${stroki}</table>
    </div>
...
window.uchKvitokHr = function(){
  const est = _vakPodtv !== null, prem = vakPremiya();
  uchKvitokOklad('HR-менеджер', VAK_OKLAD,
    `<tr><td>Закрытые вакансии: ${est ? _vakPodtv : 'не подтверждены (в Битриксе '+VAK_SPISOK.length+')'}
    ...
    'Премию даёт не Битрикс, а подтверждённое число: пока главный администратор не подтвердил месяц, ' +
    'в строке вакансий стоит прочерк. Работает неофициально, налоги с этих сумм не удерживаются.');
};
window.uchKvitokBuh = function(){
  uchKvitokOklad('Бухгалтер', 425540,
    `<tr style="background:var(--gl)"><td><b>ИТОГО К ВЫПЛАТЕ</b></td>
       <td><b style="font-size:16px;color:var(--blue)">${fmtN(425540)}</b></td></tr>`,
    'В графике вы не стоите: оклад платится по норме рабочих дней месяца целиком, пропуски ставятся числом смен ' +
    'в корректировке. Оклад указан с налогами — пенсионные, ИПН и ВОСМС вносятся в корректировке.');
};
...
accountant:[
 {t:'Начисление', otkryt:'pgPayroll',
  txt:['Ведомость собирается сама: продажи из 1С, смены из графика и отметок, сетка '
      ...
      +'и умножается на дни из графика склада; отмечаться ему не нужно.',
       'Следом два блока, где графика нет вовсе: «🧮 Бухгалтеры» — 425 540 ₸ и «🧑‍💼 HR-менеджеры» — 300 000 ₸. '
      +'Оклад там платится по норме рабочих дней целиком, а пропуски вы ставите числом смен в корректировке. '
      +'У HR сверху оклада — премия за закрытые вакансии: сверх пяти по 40 000 ₸, но только по числу, которое '
      +'подтвердил главный администратор. Красная строка в блоке значит, что он этого ещё не сделал, — '
      +'напомните, иначе человек получит один оклад.'],
 ...
 {t:'Мой расчётный лист', cel:'act:moilist',
  txt:['Внизу меню, в группе «Моё», — ваш собственный расчётный лист. Ваш оклад — 425 540 ₸ при норме '
      +'рабочих дней месяца, и в графике вы не стоите: отмечаться не нужно, оклад платится по норме целиком.',
       'Пропуски ставятся числом смен в корректировке — себе в том числе. Оклад указан с налогами: '
      +'425 540 ₸ дают на руки около 350 000 ₸, пенсионные, ИПН и ВОСМС вносятся в корректировке.'],
...
admin:[
 ...
 {t:'Вакансии HR', otkryt:'pgVakansii', cel:'act:vak-podtv',
  txt:['Пункт «Вакансии HR» в группе «Люди» показывает, сколько карточек найма перешло в этом месяце '
      +'на «В штате». Из этого числа считается премия HR-менеджера: первые пять входят в оклад, '
      +'за каждую сверх — 40 000 ₸.',
       'Число из Битрикса само в зарплату не идёт — его подтверждаете вы, и только вы. Пока подтверждения нет, '
      +'в начислении у HR стоит прочерк, а в блоке горит красная строка. Если карточку закрыли дважды или '
      +'человек не вышел — поставьте своё число и впишите комментарий, почему оно другое. В закрытом месяце '
      +'число уже не меняется.'],
  zad:'Откройте «Вакансии HR» и подтвердите число.',
  tyk:'#uch-vak-podtv,#n-pgVakansii,#s-pgVakansii'},
 {t:'Журнал',
...
KURSY.hr = [
 {t:'Ваш экран — «Вакансии»', otkryt:'pgVakansii', cel:'page:pgVakansii',
  txt:['Вы HR-менеджер. Программа открывается сразу на «Вакансиях»: меню короткое — «Вакансии», '
      +'«Мой расчётный лист», «Антирейтинг» и «Достижения». Учёт продаж вас не касается, а зарплата считается здесь же.',
       'В графике вы не стоите и не отмечаетесь: оклад 300 000 ₸ платится по норме рабочих дней месяца целиком. '
      +'Пропуск бухгалтер ставит числом смен в корректировке — тогда оклад будет пропорционально меньше.'],
 ...
 {t:'Что считается закрытой вакансией',
  txt:['Закрытая вакансия — карточка воронки найма, которая в этом месяце впервые перешла на стадию «В штате». '
      +'Число программа берёт из Битрикса сама, вручную его никто не вбивает: закрыли человека в найме — он появился здесь.',
       'Три плитки наверху читаются так: «В Битриксе» — что насчитала программа, «Подтверждено» — что принял '
      +'главный администратор, «Премия HR» — сколько за это пойдёт в зарплату. Пока в «Подтверждено» прочерк, '
      +'премии нет, сколько бы ни было в Битриксе.'],
  zad:'Посмотрите список карточек и нажмите «Дальше».'},
 {t:'Как считается премия',
  txt:['Первые пять закрытых вакансий за месяц входят в оклад. За каждую сверх пяти — 40 000 ₸. '
      +'Девять закрытых вакансий — это четыре сверх порога, то есть 160 000 ₸ к окладу.',
       'Если в Битриксе и в жизни разошлось — карточку закрыли дважды, человек не вышел, — скажите об этом '
      +'главному администратору: он подтвердит другое число и напишет комментарий. Спорить с плиткой «В Битриксе» '
      +'бесполезно, в зарплату идёт подтверждённое.'],
  zad:'Прочитайте и нажмите «Дальше».'},
 {t:'Мой расчётный лист', cel:'page:pgMoiList',
  ...
  zad:'Откройте «Мой расчётный лист».', tyk:'#n-pgMoiList,#m-pgMoiList,#s-pgMoiList'},
 {t:'Откройте сам лист', cel:'act:moilist',
  txt:['В листе две строки: оклад по норме рабочих дней и строка вакансий. Если месяц не подтверждён, '
      +'в ней так и написано: «не подтверждены (в Битриксе 9)» и прочерк вместо суммы — это не ошибка расчёта, '
      +'а несделанное подтверждение.',
