...
const ACH_KAT_U = {
  ...
  l_kursy:{st:'Сова',nm:'Курсы',kat:'Ступени',zk:'Сданный курс обучения — одна сова за каждый курс',
    fr:'Награда из белого фарфора в форме совы, сидящей на стопке книг',skoro:true},
...
const ACH_U = [
  ...
  {n:'Акишева Жанар', d:'Продавец-кассир · Асфендиярова', dost:67, g:[
   ...
   anti:[{code:'x_muha',den:'16 сентября 2026',za:'Опоздания: пять раз за две недели',srok:'ещё 4 дня'}]},
];
let _achVklU = 'lyudi';
    ...
    <div style="padding:12px 20px 18px;text-align:center">
      ...
      ${k.anti
        ? `<div style="font-size:14px;font-weight:600;color:var(--red);margin-top:14px">Простоит: ${esc(g.srok)}</div>
           ...
           <div class="rnote" style="text-align:left">Поделиться антикубком нельзя. В общий чат программа пишет о нём иначе, чем о награде:
      ...
      <div style="text-align:left;margin-top:16px">
        ${spisokStrok
          ? `<details><summary style="cursor:pointer;font-size:13px;color:var(--blue);padding:6px 0">${k.rost==='stupen'?'Пройденные ступени':'Все даты'} · ${(g.proydeno||g.daty).length}</summary>${spisokStrok}</details>`
...
window.uchAchPodelitsya = function(b,i,code){
  ...
  b.closest('.ovc').innerHTML = `<div style="padding:16px">
    ...
    <div class="rnote">Программа рисует картинку 1080×1350 — формат ленты. Личных сумм на ней нет: статуэтка, награда,
      за что она и что это за вещь. Картинка сохраняется в телефон, дальше вы отправляете её сами, куда хотите.</div></div>`;
...
seller:[
 ...
 {t:'Профиль и награды', cel:'act:achprofil',
  txt:['Нажмите на человека — откроется его карточка. Сверху счётчик: сколько видов наград собрано из тех, что доступны '
      ...
      +'простоят. Они временные и в счётчик наград не входят. «Дырявый зонт» и «Громоотвод» приходят от погоды '
      +'антирейтинга (15 и 30 штрафных баллов) и уходят сами, когда счёт опустится; остальные вручает руководство '
      +'на срок от 1 до 30 дней. В карточке антикубка написано прямо: на зарплату и награды он не влияет.'],
