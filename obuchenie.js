...
const MOB_SHORT = {'Ввод продаж':'Продажи','Отчёт магазина':'Отчёт','Начисление ЗП':'Зарплата',
  'История продавцов':'Продавцы','История отчётов':'Отчёты','Новые сотрудники':'Новые','Что нового':'Новое',
...
function menyu(){
  ...
  :[{g:'Магазины'},
    ...
    {g:'Люди'},{ic:'👤',l:'Сотрудники',fn:'pgSellers'},{ic:'👥',l:'Новые сотрудники',fn:'pgQueue'},
...
// Как в боевой · 33: очередь из 1С и Битрикса, решение одно на обе записи.
let _novRazobran = false;
...
window.pgQueue = function(){
  nav('pgQueue'); setTitle('Новые сотрудники'); setBadge(_novRazobran?'':'<span class="tag tw">2 ждут</span>');
  const sel = (id,opts) => `<select class="fc" id="${id}" style="min-width:120px">${opts.map(o=>`<option>${o}</option>`).join('')}</select>`;
  const stroka1 = _novRazobran ? '' : `<tr>
      <td><b>Жумабаева Айгерим</b> ${istU('1С','g')}<div>Айгерим Жумабаева ${istU('Битрикс','b')}</div>
        <div style="font-size:11.5px;color:var(--muted)">Продавец-кассир · отдел «Магазин Республика» · из 1С — Республика</div></td>
      <td>${fmt(1240000)}</td><td>${sel('uch-nov-rol-0',['Продавец-кассир','Раннер','Стример'])}</td>
      <td>${sel('uch-nov-mag-0',['Республика','Манаса','Асфендиярова'])}</td>
      <td>${sel('uch-nov-est-0',['— нет —','Ахметова Алия'])}</td>
      <td style="white-space:nowrap"><button class="btn bsm bp" id="uch-nov-dobavit-0" onclick="uchNovDobavit()">Добавить</button>
        <button class="btn bsm">Прикрепить</button> <button class="btn bsm">Игнорировать</button></td></tr>`;
  setContent(`<div class="card"><div class="ct">Ждут решения</div>
    ...
    ${_novRazobran?'<div class="alert as" style="display:block">✓ Карточка создана, пароль 1234. Связаны сразу и 1С, и вход из Битрикса.</div>':''}
    <div class="tw-wrap"><table><tr><th>Кто</th><th>Продажи за 30 дней</th><th>Роль у нас</th><th>Магазин у нас</th><th>Уже заведён у нас</th><th></th></tr>
      ${stroka1}
      <tr><td><b>Ким Сергей</b> ${istU('Битрикс','b')}
        <div style="font-size:11.5px;color:var(--muted)">Водитель · отдел «Склад»</div></td>
        <td>—</td><td>—</td><td>—</td><td>—</td>
        <td style="white-space:nowrap"><button class="btn bsm">Добавить</button> <button class="btn bsm">Прикрепить</button>
          <button class="btn bsm">Игнорировать</button></td></tr></table></div>
    <div class="rnote">Добавить — создать карточку и связать обе записи. Прикрепить — связать с тем, кто уже заведён.
      Игнорировать — спрятать служебные учётки. Цифра на пункте меню — сколько людей ждут.</div>
    <button class="btn" style="margin-top:.6rem">🔄 Обновить из Битрикса</button></div>
  <div class="card"><div class="ct">Уже разобраны</div><div class="tw-wrap"><table>
    <tr><th>Кто</th><th>Решение</th><th>Кто решил</th></tr>
    ${_novRazobran?`<tr><td>Жумабаева Айгерим ${istU('1С','g')} ${istU('Битрикс','b')}</td><td>добавлен</td><td>вы</td></tr>`:''}
...
window.uchNovDobavit = function(){ _novRazobran = true; tr.sobytie('act:novye'); pgQueue(); };
...
admin:[
 ...
 {t:'Новые сотрудники', cel:'act:novye',
  txt:['Сотрудников руками больше не заводят — их стыкуют. В «Новые сотрудники» приходят все, кого программа ещё '
  ...
  zad:'Откройте «Новые сотрудники» и добавьте Жумабаеву Айгерим.',
