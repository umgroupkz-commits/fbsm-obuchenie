...
const ACH_KAT_U = {
  m_prodavec:{st:'Вега',nm:'Продавец месяца',f:'пятиконечная звезда на тонкой игле',
  ...
  p_plan100:{st:'Десятка',nm:'План 100 %',f:'мишень со стрелой точно в центре',
    m:'дуб, стрела — латунь',za:'выполнение личного плана месяца'},
  p_chek:{st:'Пятнашка',nm:'Чек 15 000',f:'чековая лента с выбитым числом 15',
    m:'клён с выжженным числом',za:'средний чек месяца от 15 000 ₸'},
  n_privet:{st:'Кораблик',nm:'Добро пожаловать',f:'сложенный кораблик',
    m:'плотная бумага, покрытая лаком',za:'подключение учётной записи — ты на борту'},
  k_kassa:{st:'Сейф',nm:'Касса квартала',f:'сейф с приоткрытой дверцей',
    m:'чёрный мрамор с золотым замком',za:'кассу, сходившуюся все три месяца квартала'},
  r_den:{st:'Планка',nm:'Личный рекорд дня',f:'стойки с планкой для прыжков в высоту',
    m:'металл по числу наград; постамент — серый мрамор',
    za:'побитый собственный лучший день по продажам',rost:'chislo'},
  r_1m:{st:'Слиток дня',nm:'Миллион за смену',f:'банковский слиток с выбитой датой',
    m:'металл по числу наград; постамент — чёрный гранит',
    za:'личные продажи от 1 000 000 ₸ за один день',rost:'chislo'},
  l_prodazhi:{st:'Монетный столб',nm:'Личные продажи',f:'стопка монет — с каждой ступенью выше',
  ...
  l_smeny:{st:'Подкова',nm:'Смены',f:'подкова с гвоздями',
  ...
  l_stazh:{st:'Годовые кольца',nm:'Стаж в FBSM',f:'срез ствола с годовыми кольцами',
    m:'металл по годам; постамент — кап берёзы',
    za:'месяц, полгода и каждую годовщину работы в компании',rost:'stupen'},
  x_zont:{st:'Дырявый зонт',nm:'Дырявый зонт',f:'раскрытый зонт со сломанной спицей и дырой в куполе',
    m:'ржавая жесть; постамент — трухлявая доска',
    za:'погоду «Зонт обязателен» — 15 и больше штрафных баллов',anti:true},
  x_muha:{st:'Сонная муха',nm:'Сонная муха',f:'муха, уснувшая на будильнике',
...
const achKartochkaU = (p,i) => {
  ...
  h += `<div class="ct" style="margin:1rem 0 .6rem">Полка наград</div>
    ...
    + p.g.map((g,j) => `<button id="uch-ach-n-${j}" onclick="uchAchNagrada(${i},${j})" style="display:flex;flex-direction:column;align-items:center;gap:4px;padding:6px 2px;border:0;background:none;cursor:pointer;color:var(--text);font:inherit">
      <span style="position:relative">${achStatU(g,72)}${g.n>1?`<span style="position:absolute;right:-2px;bottom:-2px;background:var(--bl);color:var(--bd);border-radius:99px;font-size:11px;font-weight:700;padding:1px 5px">×${g.n}</span>`:''}</span>
      <span style="font-size:11.5px;text-align:center;line-height:1.25">${esc(ACH_KAT_U[g.code].st)}</span></button>`).join('')
    + `</div>`;
...
window.pgAchivki = function(){
  ...
  if(_achVklU==='ordena'){
    const vse = ['m_prodavec','p_plan100','p_chek','k_kassa','n_privet','r_1m','r_den'];
    setContent(tabs + `<div class="card" style="max-width:640px"><div class="ct">Ордена</div>
      <p style="margin:0 0 .8rem">Все награды компании. Серые — ещё никем не получены; нажмите — откроется окно ордена: за что даётся и кому, у рекордов — линейка, как растёт металл, у лестниц — статуэтка каждой ступени.</p>
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(96px,1fr));gap:10px 6px">`
      + vse.map(code => `<div style="display:flex;flex-direction:column;align-items:center;gap:4px">${achStatU({code,n:1},72)}
        <span style="font-size:11.5px;text-align:center;line-height:1.25">${esc(ACH_KAT_U[code].st)}</span></div>`).join('')
      + `</div></div>`);
    return;
  }
  if(_achVklU==='lenta'){
  ...
  setContent(tabs + `<div class="card" style="max-width:640px">` + ACH_U.map((p,i)=>`
    <button id="uch-ach-p-${i}" onclick="uchAchProfil(${i})" style="display:flex;gap:12px;align-items:center;width:100%;text-align:left;padding:10px;border:1px solid var(--border);border-radius:var(--rs);background:var(--surface);margin-bottom:8px;cursor:pointer;color:var(--text);font:inherit">
      ...
      <span style="display:flex;gap:2px;margin-top:4px;flex-wrap:wrap">${p.g.slice(0,6).map(g=>achStatU(g,26)).join('')}${(p.anti||[]).map(g=>achStatU(g,26)).join('')}</span></span></button>`).join('') + `</div>`);
};
...
seller:[
 ...
 {t:'Поделиться наградой', cel:'act:achpodel',
  txt:['Нажмите на статуэтку — откроется расшифровка: форма, материал и за что вручается. У своей награды там же есть '
      ...
      +'О новой награде программа и так пишет в общий чат Битрикса.'],
